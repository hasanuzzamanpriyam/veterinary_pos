        ...
        <table class="printable table table-bordered table-striped mb-0 w-100" cellspacing="0">
            ...
            <tr>
                @foreach ( $columns as $key => $column )
                    <th class="{{$key}}" @if (in_array($key, ['quantity', 'discount_qty', 'return_qty', 'purchase_qty']))
                        style="max-width: 60px; text-align: center; padding: 0; border-collapse: collapse;"
                    @endif>{{ $column }}</th>
                @endforeach
            </tr>
            </thead>
            <tbody>
                @foreach ($suppliers as $supplier)
                    @php
                        ...
                        $total_return_qty = [];
                        $total_purchase_qty = [];
                        // $total_tk = $total_purchases - $total_return - $total_discount + $total_carring + $total_others;
                    ...
                    <tr>
                        ...
                        <td class="supplier_phone_td phone" style="text-align: center">{{ $supplier->mobile }}</td>
                        @endif
                        ...
                        {{-- Credit Limit --}}
                        <td class="text-right supplier_credit_limit_td credit_limit" style="text-align: right">
                            {{ $supplier->credit_limit ? number_format($supplier->credit_limit) . '/=' : '' }}
                        </td>
                        @endif

                        @if ( array_key_exists("quantity", $columns) )
                        <td class="quantity text-wrap" style="text-align: center; max-width: 60px">
                        ...
                        <td class="discount_qty text-wrap" style="text-align: center;max-width: 60px">
                        ...
                        <td class="return_qty text-wrap" style="text-align: center;max-width: 60px">
                        ...
                        <td class="purchase_qty text-wrap" style="text-align: center;max-width: 60px">
                        ...
                        {{-- Weight --}}
                        <td class="text-right weight" style="text-align: right">{{ $totalWeight ? $totalWeight / 1000 . 'MT' : '' }}</td>
                        @endif
                        @if ( array_key_exists("purchase_amount", $columns) )
                        {{-- Total purchases/Price --}}
                        <td class="text-right purchase_amount" style="text-align: right">{{ $total_purchases ? number_format($total_purchases) . '/=' : '' }}</td>
                        @endif
                        ...
                        {{-- VAT --}}
                        <td class="text-right vat" style="text-align: right">{{ $total_vat ? number_format($total_vat) . '/=' : '' }}</td>
                        @endif
                        ...
                        {{-- payment --}}
                        <td class="text-right payment" style="text-align: right">{{ $total_payments ? number_format($total_payments) . '/=' : '' }}</td>
                        @endif
                        @if ( array_key_exists("total_payment", $columns) )
                        {{-- payment --}}
                        <td class="text-right total_payment" style="text-align: right">{{ $total_G_payment ? number_format($total_G_payment) . '/=' : '' }}</td>
                        @endif
                        @if ( array_key_exists("old_due", $columns) )
                        {{-- old_due --}}
                        <td class="text-right old_due" style="text-align: right">{{ $prev_due ? number_format($prev_due) . '/=' : '' }}</td>
                        @endif
            ...
            <tfoot>
                <tr>
                    ...
                    @endif
                    @if ( array_key_exists("weight", $columns) )
                    <td style="text-align: right"><b>{{$g_total_sumarry['weight'] ? $g_total_sumarry['weight'] / 1000 . 'MT' : ''}}</b></td>
                    @endif
                    @if ( array_key_exists("purchase_amount", $columns) )
                    ...
                    @endif
                    @if ( array_key_exists("old_due", $columns) )
                    <td class="text-right old_due" style="text-align: right"><b>{{$g_total_sumarry['previous_due'] ? number_format($g_total_sumarry['previous_due']) . '/=' : ''}}</b></td>
                    @endif
                    @if ( array_key_exists("balance", $columns) )
