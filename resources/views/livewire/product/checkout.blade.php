                ...
                <div class="row">
                    ...
                    <div class="col-lg-8 col-md-8 col-sm-12">
                        <div class="back_button mb-2">
                            <a href="{{route('live.product.create')}}" class="btn btn-md btn-primary float-right"> <i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-12">
                        <table class="product-data table table-striped">
                                ...
                                <tr><th>Brand</th><td>{{$brands->find($product['brand_id'])->name ?? ''}}</td></tr>
                                <tr><th>Category</th><td>{{$categories->find($product['category_id'])->name ?? ''}}</td></tr>
                                <tr><th>Product Group</th><td>{{$product_groups->find($product['group_id'])->name ?? ''}}</td></tr>
                                <tr><th>Type</th><td>{{ucfirst($product['type'])}}</td></tr>
                            </table>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-12">
                            <table class="product-data table table-striped">
                                <tr><th>Size</th><td>{{$sizes->find($product['size_id'])->description ?? ''}}</td></tr>
                                <tr><th>Alert Quantity</th><td>{{$product['alert_quantity'] ?? ''}}</td></tr>
                                <tr><th>Purchase Rate</th><td>{{$product['purchase_rate'] ? $product['purchase_rate'] : 0}}/=</td></tr>
                                <tr><th>MRP Rate</th><td>{{$product['mrp_rate'] ? $product['mrp_rate'] : 0}}/=</td></tr>
                                <tr><th>Sale Rate</th>  <td>{{$product['price_rate'] ? $product['price_rate'] : 0}}/=</td></tr>
