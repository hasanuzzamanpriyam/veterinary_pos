@section('page-title', 'Bank '. $view .' Add' )

<div class="col-md-12 col-sm-12 ">
    <div class="x_panel">
        <div class="x_title px-3">
            <div class="header-title d-flex align-items-center gap-2">
                <h2>Add {{ucfirst($view)}}</h2>
                        ...
                        <div class="row justify-content-end">
                            <div class="search-area col-lg-12 col-md-12 col-sm-12 text-left purchase_return_entry_supplier_col">
                                <div wire:ignore class="row">
                                    ...
                                    <div class="col-md-9">
                                        <select id="bank_id" class="d-block py-1 border" class="form-control" required>
                                            <option value="">Select Bank</option>
                                            @foreach ($banks as $bank )
                                                <option value="{{ $bank->id }}">
                                                    {{ $bank->name }} -
                                                    {{ $bank->branch }} -
                                                    {{ Str::upper($bank->ac_mode) }} -
                                                    {{ $bank->account_no }}
                                                </option>
                                            @endforeach
                                        </select>
                ...
                <div class="row collection-form-area d-flex justify-content-center">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-12">
                        <div class="form-group">
                            ...
                            <input type="text" value="{{ formatAmount($bank_info->balance ?? 0) }}" class="form-control text-right" required readonly>
                        </div>
                    </div>
                </div>
                <div class="row collection-form-area d-flex justify-content-center">
                    ...
                    <div class="col-lg-4 col-md-2 col-sm-12">
                        <div class="form-group">
                            ...
                            <select id="payment_by" type="text" name="payment_by" class="form-control" required>
                            ...
                            @if (in_array($payment_by, $list_of_banking_service))
                                <select id="account_title" type="text" name="account_title" class="form-control">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-12">
                        <div class="form-group">
                            ...
                            <input type="text" wire:keyup="amoutCalculation($event.target.value)" class="form-control text-right" required>
                        </div>
                    </div>
                </div>
                <div class="row collection-form-area d-flex justify-content-center">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-12">
                        <div class="form-group">
                            @php
                                $total = $view == 'withdraw' ? $balance - $amount : $balance + $amount;
                            @endphp
                            <label  for="prepare" class="collection_entry_lebel py-1">Total</label>
...
$(document).ready(function(){
    ...
    $('#bank_id').select2({
        placeholder: 'Select Bank from here',
    });

    $('#bank_id').on('change', function(e) {
        @this.handleChange('bank_id', e.target.value);
    });

    $('#payment_by, #account_title').select2();

    $('#payment_by').on('change', function(e) {
        @this.handleChange('payment_by', e.target.value);
    });
    $('#account_title').on('change', function(e) {
        @this.handleChange('payment_by_bank', e.target.value);
    });
