...
class Index extends Component
{
    ...
    public function supplierDueSearch()
    {
        ...
        // dd($this->get_supplier_id, $this->startDate, $this->endDate);
        ...
        if ($this->get_supplier_id && ($this->startDate && $this->endDate)) {
            ...
            $this->purchaseProducts = SupplierTransactionDetails::where('supplier_id', $this->get_supplier_id)
        ...
        if ($this->startDate && $this->endDate && $this->get_supplier_id) {
            ...
            $bonuses = $this->purchaseProducts
                ->groupBy(function ($transaction) {
                    return \Carbon\Carbon::parse($transaction->supplier_id); // ✅ প্রথমে সাপ্লাইয়ার অনুযায়ী গ্রুপিং
                })
                ->map(function ($group) {
                    $first = $group->first();
