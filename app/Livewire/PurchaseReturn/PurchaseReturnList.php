...
class PurchaseReturnList extends Component
{
    ...
    public function render()
    {
        if ($this->start_date != null && $this->end_date != null){
            $query = SupplierLedger::where('type', 'return')
        ...
        $products = SupplierTransactionDetails::where('transaction_type', 'return')
            ->whereIn('transaction_id', $ledgerIds)
            ->orderBy('id', 'DESC')
            ->get();

        return view('livewire.purchase-return.purchase-return-list', get_defined_vars())
